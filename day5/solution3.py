...
def update_range(lower_bound, upper_bound):
      global valid_ranges
      global valid_range_no_overlap
      # Check if the upper or lower bound is in between a range
      # if it is then update the range
      for index, valid_range in enumerate(valid_range_no_overlap):
          vr_bounds = valid_range.split("-")
          vr_lower_bound = vr_bounds[0]
          vr_upper_bound = vr_bounds[1]
          last_item_in_range = valid_range == valid_range_no_overlap[-1]

          if int(lower_bound) >= int(vr_lower_bound) and int(upper_bound) <= int(vr_upper_bound):
              break
          #lower bound in range
          elif int(vr_upper_bound) >= int(lower_bound) >= int(vr_lower_bound):
              # low <= num <= high
              # change the upper bound in that range
              valid_range_no_overlap[index] = f"{vr_lower_bound}-{upper_bound}"
              # each time we update something, check that we didn't mess up
              # check_itself()
              continue
          #upper bound in range
          elif int(vr_lower_bound) <= int(upper_bound) <= int(vr_upper_bound):
              # if only the upper bound is in the list
              # change the lower bound in that range
              valid_range_no_overlap[index] = f"{lower_bound}-{vr_upper_bound}"
              # check_itself()
              continue
          #neither bound in range
          else:
              if last_item_in_range:
                  print("Neither bound in any element in the list: ", lower_bound, upper_bound)
                  # ONLY DO THIS IF IT IS THE LAST ITEM IN THE LIST
                  valid_range_no_overlap.append(lower_bound + "-" + upper_bound)
                  break
...
with open(filename, "r") as file_object:
    ...
    # start with a the first range and compare to the rest
    valid_range_no_overlap.append(valid_ranges[0])
...
# fixme: count how many valid ids are here
check_itself()
...
# 349437839019715 too low
# 345189003074454, too low
# 418742536707923, too high
# 288124828977028, too low
# 333289213565809, too low
# 362469684139099 not right - grr