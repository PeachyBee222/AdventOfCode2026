...
filename = "/mnt/shares/h/adventofcode/day3/testdata.txt"
final_number = 0
overall_total  = 0
one_digit = 0
two_digit = 0
...
with open(filename, "r") as file_object:
    ...
    for line in file_object:
        ...
        for index, digit_char in enumerate(line):
            ...
            # The last digit can only update the twelve digit
            # print("Checking digit ", digit_char, " at index ", index, " with length ", length-12)

            if index >= length - 12: # skip the last 11 digits since they can't be reset
                if int(twelve_digit) < num:
                    twelve_digit = digit_char
            # modify the digits based on their position
            elif int(one_digit) < num and index <= length - 12: # increase the front digit and change the remaining digits to the following numbers
                print("Update 1 digit from ", one_digit, " to ", num, " at index ", index)
                update_digits(line, index, 1)
                ## FIXME: i dont think this will work
            elif int(two_digit) < num and index <= length - 11 and ??? in range(index + 1, length): # only increase the 2s digit if the 1s digit doesn't change
                print("Update 2 digit from ", two_digit, " to ", num, " at index ", index)
                update_digits(line, index, 2)
            elif int(three_digit) < num and index <= length - 10: # only increase the 3s digit if the 2s digit doesn't change
                print("Update 3 digit from ", three_digit, " to ", num, " at index ", index)
                update_digits(line, index, 3)
            elif int(four_digit) < num and index <= length - 9: # only increase the 4s digit if the 3s digit doesn't change
                print("Update 4 digit from ", four_digit, " to ", num, " at index ", index)
                update_digits(line, index, 4)
            elif int(five_digit) < num and index <= length - 8: # only increase the 5s digit if the 4s digit doesn't change
                print("Update 5 digit from ", five_digit, " to ", num, " at index ", index)
                update_digits(line, index, 5)
            elif int(six_digit) < num and index <= length - 7: # only increase the 6s digit if  the 5s digit doesn't change
                print("Update 6 digit from ", six_digit, " to ", num, " at index ", index)
                update_digits(line, index, 6)
            elif int(seven_digit) < num and index <= length - 6: # only increase the 7s digit if the 6s digit doesn't change
                print("Update 7 digit from ", seven_digit, " to ", num, " at index ", index)
                update_digits(line, index, 7)
            elif int(eight_digit) < num and index <= length - 5: # only increase the 8s digit if the 7s digit doesn't change
                print("Update 8 digit from ", eight_digit, " to ", num, " at index ", index)
                update_digits(line, index, 8)
            elif int(nine_digit) < num and index <= length - 4: # only increase the 9s digit if the 8s digit doesn't change
                print("Update 9 digit from ", nine_digit, " to ", num, " at index ", index)
                update_digits(line, index, 9)
            elif int(ten_digit) < num and index <= length - 3: # only increase the 10s digit if the 9s digit doesn't change
                print("Update 10 digit from ", ten_digit, " to ", num, " at index ", index)
                update_digits(line, index, 10)
            elif int(eleven_digit) < num and index <= length - 2: # only increase the 11s digit if the 10s digit doesn't change
                print("Update 11 digit from ", eleven_digit, " to ", num, " at index ", index)
                update_digits(line, index, 11)
            elif int(twelve_digit) < num and index <= length - 1: # only increase the 12s digit if the 11s digit doesn't change
